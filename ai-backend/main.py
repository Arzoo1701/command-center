...
from datetime import datetime # ✨ NEW: Required for activity timestamps
...
# Disable TensorFlow warnings in the terminal
os.environ["TF_CPP_MIN_LOG_LEVEL"] = "3" 

# ==========================================
# 🚀 INITIALIZE APP (Must be at the top!)
# ==========================================
...
# ✨ NEW: Helper function to silently log actions
...
# 🤖 3. AI SETUP
# ==========================================
print("Waking up Embedding Model...")
embeddings = HuggingFaceEmbeddings(model_name="all-MiniLM-L6-v2")
vector_store = Chroma(embedding_function=embeddings, persist_directory="./chroma_db")
os.makedirs("temp_uploads", exist_ok=True)

# 🚨 PASTE YOUR GEMINI API KEY HERE
GOOGLE_API_KEY = "AIzaSyCEWEb4QZEF4ZJklp4NpJiLfpltVFpnBV0"
...
# ==========================================
# 🧪 5. TEST REGISTRY FOR LEETCODE SCALING
# ==========================================
TEST_REGISTRY = {
    "python": {
...
# ✨ NEW: The Analytics Aggregation Endpoint
...
async def get_full_activity_log(limit: int = 100, db: Session = Depends(get_db)):
    try:
        # Fetch the latest 100 logs, newest first
...
async def upload_document(file: UploadFile = File(...)):
    try:
        ...
        elif file.filename.endswith(".txt") or file.filename.endswith(".md"): loader = TextLoader(file_path)
        else:
            os.remove(file_path)
        ...
        os.remove(file_path)
        return {"filename": file.filename, "status": "Vectorized and Stored in ChromaDB", "chunks_created": len(chunks)}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def inspect_chunks(filename: str, limit: int = 10, offset: int = 0):
    try:
        results = vector_store.get(where={"source": f"temp_uploads/{filename}"})
        all_docs = results["documents"]
        return {"filename": filename, "chunks": all_docs[offset : offset + limit], "total_chunks": len(all_docs), "has_more": (offset + limit) < len(all_docs)}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
